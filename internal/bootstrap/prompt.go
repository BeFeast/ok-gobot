...
func BuildPrompt(loader *Loader, registry *tools.Registry, opts PromptOptions) string {
	...
	if mode == "full" {
		...
		prompt.WriteString("For Telegram-facing replies, use plain text. Do not wrap slash commands, file paths, or links in Markdown backticks or bold markers.\n")
		prompt.WriteString("When summarizing remembered facts, prefer 2-5 short bullets over long explanatory paragraphs, and do not end with a generic help offer.\n\n")
