...
func (t *HostTaskTool) Description() string {
	return "Spawn a host-operations worker to carry out a task on the machine: install packages, " +
		...
		"run immediately (no approval). A snapshot of the workspaces is taken before the first " +
		"state-changing command so the task is revertable. Use this to actually perform host " +
		"operations instead of writing a runbook and stopping."
}
...
REVERT SAFETY: the exec tool takes an automatic snapshot of the workspaces before your first state-changing command, so this whole task can be reverted.

HOW YOU WORK:
- Use the exec tool to run shell commands on the host. Commands run immediately — there is no approval step.
- Prefer the smallest, most reversible step. exec snapshots the workspaces before your first change and also backs up each existing file a command overwrites.
...
- Return a concise summary: what changed, what was verified (with rc/output), and how to revert if needed.`, task)
