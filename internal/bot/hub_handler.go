...
func (b *Bot) processViaHubWithContent(
	...
	// Chat replies keep a protected fresh transcript tail so the latest turns
	// always survive trimming, while older history remains token-aware.
	history := b.loadChatHistory(sessionKey, b.getEffectiveModel(chatID))
